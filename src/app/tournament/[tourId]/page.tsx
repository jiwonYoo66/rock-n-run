        ...
        <R.DetailWrapper>
          ...
          <R.MobileWrapper>
            <Swiper slidesPerView={1} className="mySwiper">
              {[
                "/assets/dummy/tour_post1.png",
                "/assets/dummy/reservation_map1.svg",
              ].map((data, index) => (
                <SwiperSlide key={index}>
                  <R.ImageWrapper>
                    <Image src={data} alt="프로젝트 소개 이미지" fill />
                  </R.ImageWrapper>
                </SwiperSlide>
              ))}
            </Swiper>
          ...
          <R.ImageArea>
            ...
            <R.ImageWrapper>
              <Image
                src="/assets/dummy/reservation_map1.svg"
                alt="프로젝트 소개 이미지"
                fill
              />
            </R.ImageWrapper>
          ...
          <R.InfoWrapper>
            <R.InfoBlock>
              ...
              <R.InfoContent>2025.09.25(목) ~ 2025.09.28(일)</R.InfoContent>
            </R.InfoBlock>
            <R.InfoBlock>
              <R.InfoHeader>장소</R.InfoHeader>
              <R.InfoContent>전라북도 장수군 장수읍 12-1</R.InfoContent>
            </R.InfoBlock>
            <R.InfoBlock>
              <R.InfoHeader>진행종목</R.InfoHeader>
              <R.InfoContent>100M, 100K, 38K-J, 38K-P, 20K</R.InfoContent>
            </R.InfoBlock>
            <R.InfoBlock>
              ...
              <R.InfoListWrapper>
                {COURSE_DATA.map((data, index) => (
                  <R.InfoList key={index}>
                    ...
                    <R.Info $disc>
                      {data?.recruitment.toLocaleString()}명
                    </R.Info>
                  </R.InfoList>
                ))}
              </R.InfoListWrapper>
            ...
            <R.InfoBlock>
              ...
              <R.InfoListWrapper>
                {PRICE_DATA.map((data, index) => (
                  <R.InfoList key={index}>
                    <R.Info>{data?.type}</R.Info>
                    <R.Info $disc>{data?.price.toLocaleString()}명</R.Info>
                  </R.InfoList>
                ))}
              </R.InfoListWrapper>
            </R.InfoBlock>
                ...
                <R.InfoWrapper>
                  {PRODUCT_DATA.map((data, index) => (
                    <R.ProductWrapper key={index}>
                      <R.ProductBadge>{data?.name}</R.ProductBadge>
                      {data?.price !== 0 && (
                        <R.ProductPrice>
                          정가 {data?.price.toLocaleString()}원
                        </R.ProductPrice>
                      )}
                      <R.InfoDetail
                        key={index}
                        $padding={data?.price ? "45px 8px 0" : "73px 8px 0"}
                      >
                        <Image
                          src={data?.url}
                          alt="필수 장비 이미지"
        ...
        <R.MobileSubmit>
          ...
          {showMobileCalendar ? (
            <StyledButton
              title="신청하기"
              onClick={() =>
                router.push(`/${paths.TOURNAMENT}/${tourId}/${paths.COURSE}`)
              }
              fontSize={18}
            />
          ) : (
